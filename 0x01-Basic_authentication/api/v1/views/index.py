#!/usr/bin/env python3
""" Module of Index views
"""
from flask import abort
from api.v1.views import app_views
from flask import jsonify, abort
from api.v1.views import app_views


@app_views.route('/status', methods=['GET'], strict_slashes=False)
def status() -> str:
    """ GET /api/v1/status
    Return:
      - the status of the API
    """
    return jsonify({"status": "OK"})


@app_views.route('/stats/', strict_slashes=False)
def stats() -> str:
    """ GET /api/v1/stats
    Return:
      - the number of each objects
    """
    from models.user import User
    stats = {}
    stats['users'] = User.count()
    return jsonify(stats)


@app_views.route('/api/v1/unauthorized', methods=['GET'], strict_slashes=False)
def unauthorized():
    """
    This function is a route handler for the '/api/v1/unauthorized' endpoint.
    It is responsible for handling
    GET requests and returning a 401 Unauthorized status code.
    """
    abort(401)


@app_views.route('/api/v1/forbidden', methods=['GET'], strict_slashes=False)
def forbidden():
    """
    Raises a 403 Forbidden error.

    This function is used to raise a 403
    Forbidden error, indicating
    that the user does not have permission to access the requested resource.

    :return: None
    """
    abort(403)
